...
<html>
  ...
  <body>
    <header id="encabezadoPrincipal">
      ...
      <?php
     $fecha_inicio=strtotime( $_GET["primeraFecha"]);
     $fecha_final=strtotime($_GET["segundaFecha"]);
     $link = mysqli_connect('localhost','root')
          or die('No se pudo conectar: ' . mysql_error());
      mysqli_select_db($link,'couchinn') or die('No se pudo seleccionar la base de datos');
      $consul="SELECT * FROM pagos";
      ...
      if($datos==false){
        ...
        <table class="tabla" border="3" >
          ...
          <?php while($fila=mysqli_fetch_row($datos)){
                   $f_actual=strtotime($fila['2']);
                  if($f_actual>=$fecha_inicio && $f_actual<=$fecha_final){
                    ?>
                    <tr>
